...
class StoreType {
	...
	public static $num_mutable_references = 5;
	public static $num_path_limiting_references = 5;
	public static $num_path_limiting_sequence = 50;
	
	protected static $stmt_object_description_serial = null;
	protected static $stmt_object_sub_description_serial = null;
	...
	const TYPE_NAMESPACE_SCHEMA = 'SCHEMA';
	...
	public function store($arr_details, $arr_definitions, $arr_object_descriptions, $arr_object_subs_details) {
		...
		DB::startTransaction('store_type');
		...
		if ($this->mode == self::MODE_OVERWRITE || ($this->mode == self::MODE_UPDATE && $arr_details !== null)) {
			...
			if ($num_type_class == self::TYPE_CLASS_REVERSAL) {
				...
				$num_type_mode = bitUpdateMode($num_type_mode, BIT_MODE_ADD, ($arr_details['reversal_mode'] == static::TYPE_MODE_REVERSAL_COLLECTION ? static::TYPE_MODE_REVERSAL_COLLECTION : static::TYPE_MODE_REVERSAL_CLASSIFICATION));
			}
		...
		if ($arr_definitions) {
			...
			foreach ($arr_definitions as $arr_definition) {
				...
				$arr_definition['definition_id'] = $this->getTypeDefinitionID($this->type_id, $arr_definition['definition_id']);
		...
		if ($num_type_class == self::TYPE_CLASS_REVERSAL) {
			...
			if ($this->mode == self::MODE_OVERWRITE || ($this->mode == self::MODE_UPDATE && $arr_details !== null)) {
				...
				if (bitHasMode($num_type_mode, static::TYPE_MODE_REVERSAL_CLASSIFICATION)) {
					...
					$sql_update = '';
					$sql_check_duplicate = "id_id = ".StoreType::getSystemTypeObjectDescriptionID(StoreType::getSystemTypeID('reversal'), 'reference')." AND type_id = ".$this->type_id;
					
					$res_check = DB::query("SELECT TRUE
							FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
						WHERE ".$sql_check_duplicate."
						LIMIT 1
					");
										
					if ($res_check->getRowCount()) {
						
						$sql_update = "UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." SET
								ref_type_id =  ".(int)$reversal_ref_type_id.",
								in_name = ".DBFunctions::escapeAs($reversal_ref_type_id, DBFunctions::TYPE_BOOLEAN)."
							WHERE ".$sql_check_duplicate;
					} else {
						
						$sql_update = "INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
					...
					$res = DB::queryMulti("
						".$sql_update."
						;
						DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
							WHERE type_id = ".$this->type_id." AND id_id = ".StoreType::getSystemTypeObjectDescriptionID(StoreType::getSystemTypeID('reversal'), 'module')."
						;
					");
				} else {
					
					$reversal_module_settings = value2JSON([
						'resource_path' => (bool)$arr_details['reversal_resource_path']
					]);
					
					$sql_update = '';
					$sql_check_duplicate = "id_id = ".StoreType::getSystemTypeObjectDescriptionID(StoreType::getSystemTypeID('reversal'), 'module')." AND type_id = ".$this->type_id;
					...
					if ($res_check->getRowCount()) {
						...
						$sql_update = "UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." SET
								value_type_settings = '".DBFunctions::strEscape($reversal_module_settings)."'
							WHERE ".$sql_check_duplicate;
					} else {
						
						$sql_update = "INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
							(id_id, type_id, value_type_settings, ref_type_id, in_name)
								VALUES
							(".StoreType::getSystemTypeObjectDescriptionID(StoreType::getSystemTypeID('reversal'), 'module').", ".$this->type_id.", '".DBFunctions::strEscape($reversal_module_settings)."', 0, FALSE)
						";
					}
					$res = DB::queryMulti("
						".$sql_update."
						;
						DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
							WHERE type_id = ".$this->type_id." AND id_id = ".StoreType::getSystemTypeObjectDescriptionID(StoreType::getSystemTypeID('reversal'), 'reference')."
						;
					");
				}
			}
				...
				foreach ($arr_object_descriptions as $arr_object_description) {
					...
					if (!$arr_object_description['object_description_name']) {
						...
						if ($this->mode == self::MODE_UPDATE && $arr_object_description['object_description_id']) {
							$arr_ids[] = (int)$arr_object_description['object_description_id'];
						}
					...
					$object_description_ref_type_id = $arr_object_description['object_description_ref_type_id'];
										
					$this->parseTypeObjectDescriptionValueType($object_description_value_type_base, $object_description_ref_type_id);
					
					$arr_value_type_base = $arr_value_types_base[$object_description_value_type_base];
					...
					if ($object_description_value_type_base == 'reversal') {
						$object_description_has_multi = true;
					} else if ($arr_value_type_base['has_support_multi']) {
					...
					if ($arr_object_description['object_description_is_identifier'] !== null && $arr_value_type_base['has_support_identifier']) {
					...
					if ($arr_value_type_base['has_support_text'] === false) {
						$object_description_in_name = false;
						$object_description_in_search = false;
					}
					...
					$object_description_id_id_ref_type_ids = null;
					if (is_array($object_description_ref_type_id)) {
						$object_description_id_id_ref_type_ids = $object_description_ref_type_id;
						$object_description_ref_type_id = 0;
					}
					
					if ($arr_object_description['object_description_id']) {
						
						$object_description_id = (int)$arr_object_description['object_description_id'];
						
						$arr_convert_objects = $this->getConvertTypeObjectDefinitions($object_description_id, $object_description_value_type_base);
						...
						$res = DB::query("UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." SET
								...
								in_search = ".DBFunctions::escapeAs($object_description_in_search, DBFunctions::TYPE_BOOLEAN).",
						...
						if ($arr_convert_objects) {
							...
							timeLimit(300);
							self::$arr_types_storage[$this->type_id] = null; // Reload type set on next call
						...
						$res = DB::query("INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
							...
							(".$this->type_id.",
								...
								".DBFunctions::escapeAs($object_description_in_name, DBFunctions::TYPE_BOOLEAN).",
								".DBFunctions::escapeAs($object_description_in_search, DBFunctions::TYPE_BOOLEAN).",
						...
						$object_description_id = DB::lastInsertID();
					}
					
					if ($this->mode == self::MODE_OVERWRITE) {
						$arr_ids[] = $object_description_id;
					}
					
					if ($object_description_id_id_ref_type_ids) {
						
						$arr_sql_id_id = [];
						$num_sort_id_id = 0;
						
						foreach ($object_description_id_id_ref_type_ids as $reference_type_id) {
							
							$arr_sql_id_id[] = "INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
								(id_id, type_id, ref_type_id, sort)
									VALUES
								(".(int)$object_description_id.", ".$this->type_id.", ".(int)$reference_type_id.", ".$num_sort_id_id.")
								".DBFunctions::onConflict('id_id, type_id, ref_type_id', ['sort'])."
							";
						
							$num_sort_id_id++;
						}
						
						$sql_id_id = implode(';', $arr_sql_id_id);
						
						$res = DB::queryMulti("
							".$sql_id_id.";
							DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
								WHERE id_id = ".(int)$object_description_id."
									AND type_id = ".$this->type_id."
									AND ref_type_id NOT IN (".implode(',', $object_description_id_id_ref_type_ids).")
							;
							SELECT id FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')."
								WHERE id_id = ".(int)$object_description_id."
									AND type_id = ".$this->type_id."
							;
						");
						
						if ($this->mode == self::MODE_OVERWRITE) { // Keep additional/supporting description ids
							
							$use_res = end($res);
							
							while ($arr_row = $use_res->fetchRow()) {
								$arr_ids[] = (int)$arr_row[0];
							}
						}
					}
			...
			if ($this->mode == self::MODE_UPDATE) {
				if ($arr_ids) {
					$sql_select = "(id IN (".implode(',', $arr_ids).") OR id_id IN (".implode(',', $arr_ids)."))";
				}
			} else {
				if ($arr_ids) {
					$sql_select = "id NOT IN (".implode(',', $arr_ids).")";
				} else {
					$sql_select = "TRUE";
				}
			}
			...
			foreach ($arr_object_subs_details as $arr_object_sub_details) {
				...
				if (!$arr_object_sub_details['object_sub_details']['object_sub_details_name']) {
					...
					if ($this->mode == self::MODE_UPDATE && $arr_object_sub_details['object_sub_details']['object_sub_details_id']) {
						$arr_ids[] = (int)$arr_object_sub_details['object_sub_details']['object_sub_details_id'];
					}
				...
				if (isset($arr_object_sub_details_self['object_sub_details_date_usage'])) {
					$object_sub_details_date_usage = $arr_object_sub_details_self['object_sub_details_date_usage'];
				} else {
					$object_sub_details_date_usage = ($arr_object_sub_details_self['object_sub_details_date_use_object_sub_details_id'] ? 'object_sub_details' : ($arr_object_sub_details_self['object_sub_details_date_start_use_object_sub_description_id'] ? 'object_sub_description' : ($arr_object_sub_details_self['object_sub_details_date_start_use_object_description_id'] ? 'object_description' : '')));
				}
				...
				if (isset($arr_object_sub_details_self['object_sub_details_location_usage'])) {
					$object_sub_details_location_usage = $arr_object_sub_details_self['object_sub_details_location_usage'];
				} else {
					$object_sub_details_location_usage = ($arr_object_sub_details_self['object_sub_details_location_use_object_id'] ? 'object' : ($arr_object_sub_details_self['object_sub_details_location_use_object_description_id'] ? 'object_description' : ($arr_object_sub_details_self['object_sub_details_location_use_object_sub_details_id'] ? 'object_sub_details' : ($arr_object_sub_details_self['object_sub_details_location_use_object_sub_description_id'] ? 'object_sub_description' : ''))));
				}				
					...
					foreach ($arr_object_sub_details['object_sub_descriptions'] as $arr_object_sub_description) {
						...
						if (!$arr_object_sub_description['object_sub_description_name']) {
							...
							if ($this->mode == self::MODE_UPDATE && $arr_object_sub_description['object_sub_description_id']) {
								$arr_sub_description_ids[] = (int)$arr_object_sub_description['object_sub_description_id'];
							}
						...
						$object_sub_description_ref_type_id = $arr_object_sub_description['object_sub_description_ref_type_id'];
						
						$this->parseTypeObjectDescriptionValueType($object_sub_description_value_type_base, $object_sub_description_ref_type_id);
						
						$arr_value_type_base = $arr_value_types_base[$object_sub_description_value_type_base];
						...
						if ($arr_value_type_base['has_support_text'] === false) {
							$object_sub_description_in_name = false;
							$object_sub_description_in_search = false;
						}
						...
						$object_sub_description_id_id_ref_type_ids = null;
						if (is_array($object_sub_description_ref_type_id)) {
							$object_sub_description_id_id_ref_type_ids = $object_sub_description_ref_type_id;
							$object_sub_description_ref_type_id = 0;
						}
						
						if ($arr_object_sub_description['object_sub_description_id']) {
							
							$object_sub_description_id = (int)$arr_object_sub_description['object_sub_description_id'];
							
							$arr_convert_objects = $this->getConvertTypeObjectSubDefinitions($object_sub_details_id, $object_sub_description_id, $object_sub_description_value_type_base);
							...
							$res = DB::query("UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')." SET
									...
									in_search = ".DBFunctions::escapeAs($object_sub_description_in_search, DBFunctions::TYPE_BOOLEAN).",
							...
							$res = DB::query("INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')."
								...
								(".(int)$object_sub_details_id.",
									...
									".DBFunctions::escapeAs($object_sub_description_in_name, DBFunctions::TYPE_BOOLEAN).",
									".DBFunctions::escapeAs($object_sub_description_in_search, DBFunctions::TYPE_BOOLEAN).",
							...
							$object_sub_description_id = DB::lastInsertID();
						}
						
						if ($this->mode == self::MODE_OVERWRITE) {
							$arr_sub_description_ids[] = $object_sub_description_id;
						}
						
						if ($object_sub_description_id_id_ref_type_ids) {
							
							$arr_sql_id_id = [];
							$num_sort_id_id = 0;
							
							foreach ($object_sub_description_id_id_ref_type_ids as $reference_type_id) {
								
								$arr_sql_id_id[] = "INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')."
									(id_id, object_sub_details_id, ref_type_id, sort)
										VALUES
									(".(int)$object_sub_description_id.", ".(int)$object_sub_details_id.", ".(int)$reference_type_id.", ".$num_sort_id_id.")
									".DBFunctions::onConflict('id_id, object_sub_details_id, ref_type_id', ['sort'])."
								";
							...
							$res = DB::queryMulti("
								".$sql_id_id.";
								DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')."
									WHERE id_id = ".(int)$object_sub_description_id."
										AND object_sub_details_id = ".(int)$object_sub_details_id."
										AND ref_type_id NOT IN (".implode(',', $object_sub_description_id_id_ref_type_ids).")
								;
								SELECT id FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')."
									WHERE id_id = ".(int)$object_sub_description_id."
										AND object_sub_details_id = ".(int)$object_sub_details_id."
								;
							");
							...
							if ($this->mode == self::MODE_OVERWRITE) { // Keep additional/supporting description ids
								...
								while ($arr_row = $use_res->fetchRow()) {
									$arr_sub_description_ids[] = (int)$arr_row[0];
								}
							}
						}
				...
				$sql_select = null;
				if ($this->mode == self::MODE_UPDATE) {
					if ($arr_sub_description_ids) {
						$sql_select = "(id IN (".implode(',', $arr_sub_description_ids).") OR id_id IN (".implode(',', $arr_sub_description_ids)."))";
					}
				} else {
					if ($arr_sub_description_ids) {
						$sql_select = "id NOT IN (".implode(',', $arr_sub_description_ids).")";
					} else {
						$sql_select = "TRUE";
					}
				}
				...
				if ($sql_select) {
					...
					$res = DB::query("DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')."
						WHERE object_sub_details_id = ".(int)$object_sub_details_id."
							AND ".$sql_select."
					");
				}
		...
		DB::commitTransaction('store_type');
		...
		self::$arr_types_storage[$this->type_id] = null; // Invalidate Type configuration cache
	...
	public function checkStore($arr_details, $arr_object_descriptions, $arr_object_subs_details) {
		...
		foreach ($arr_object_descriptions as $arr_object_description) {
			...
			if ($arr_object_description['object_description_in_name'] && !($this->type_id && $reference_type_id == $this->type_id)) { // Self-reference does not count
				$has_name = true;
			}
			...
			if ($reference_type_id) {
				...
				if (is_array($reference_type_id)) {
					$arr_reference_type_ids += $reference_type_id;
				} else {
					$arr_reference_type_ids[$reference_type_id] = $reference_type_id;
				}
			}
		}
		...
		if ($num_type_class != self::TYPE_CLASS_REVERSAL) {
			...
			if ($num_count_mutable > 3) {
				
				Labels::setVariable('what', getLabel('msg_model_reference_mutable_overflow'));
				error(getLabel('msg_model_illogical'));
			}
		}
		...
		foreach ($arr_object_subs_details as $arr_object_sub_details) {
			...
			if ($arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_type_id']) {
				
				$reference_type_id = (int)$arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_type_id'];
				$arr_reference_type_ids[$reference_type_id] = $reference_type_id;
			}
			
			$num_count_mutable = 0;
			
			foreach ((array)$arr_object_sub_details['object_sub_descriptions'] as $arr_object_sub_description) {
	...
	protected function parseTypeObjectDescriptionValueType(&$str_value_type_base, &$ref_type_id, $do_check = false) {
		...
		if ($str_value_type_base == 'reference_mutable') {
			...
			foreach ((array)$ref_type_id as $type_id) {
				...
				$store_ref_type_id[$type_id] = $type_id;
			}
			
			if ($do_check) {
				
				$num_references = count($store_ref_type_id);
				
				if ($num_references < 2 || $num_references > static::$num_mutable_references) {
				
					Labels::setVariable('what', getLabel('msg_model_reference_mutable_invalid'));
					error(getLabel('msg_model_illogical'));
				}
			}
	...
	protected function parseTypeObjectDescriptionValueTypeOptions($str_value_type_base, $ref_type_id, $has_multi, $has_default_value, $in_name, $arr_value_type_settings) {
		...
		if ($arr_value_type_settings) {
			...
			if ($has_default_value && $value_default !== null) {
					
				if ($ref_type_id) {
					
					if (is_array($ref_type_id)) {
						
						$value_default = arrParseRecursive($value_default, function($str_identifier) {
							
							list($check_type_id, $check_object_id) = explode('_', $str_identifier);
							
							return (int)$check_type_id.'_'.(int)$check_object_id;
						});
					} else {
				...
				if ($value_default !== null) {
					...
					if ($has_multi) {
						...
						$value_default = (is_array($value_default) ? array_values($value_default) : (array)$value_default);
						$value_default = array_unique(array_filter($value_default));
	...
	public static function getTypeObjectDescriptionSerialNext($type_id, $object_description_id) {
		...
		if (static::$stmt_object_description_serial === null) {
			
			DB::setConnection(null, DB::MODE_CONNECT_SECONDARY); // Use secondary connection, outside any transaction
			
			if (DB::ENGINE_IS_MYSQL) {
			
				static::$stmt_object_description_serial[0] = DB::prepare("
					UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." SET
						value_type_serial = (@num_serial := COALESCE(value_type_serial, 0) + 1)
					WHERE id = ".DBStatement::assign('object_description_id', 'i')."
						AND type_id = ".DBStatement::assign('type_id', 'i')."
				");
				static::$stmt_object_description_serial[1] = DB::prepare("
					SELECT @num_serial
				");
			} else {
				
				static::$stmt_object_description_serial = DB::prepare("
					UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." SET
		...
		if (DB::ENGINE_IS_MYSQL) {
			...
			static::$stmt_object_description_serial[0]->bindParameters(['type_id' => (int)$type_id, 'object_description_id' => (int)$object_description_id]);
			static::$stmt_object_description_serial[0]->execute();
			
			$res = static::$stmt_object_description_serial[1]->execute();
		} else {
			
			static::$stmt_object_description_serial->bindParameters(['type_id' => (int)$type_id, 'object_description_id' => (int)$object_description_id]);
			$res = static::$stmt_object_description_serial->execute();
		}
		
		$num_serial = $res->fetchRow();
		$num_serial = (int)$num_serial[0];
	...
	public static function getTypeObjectSubDescriptionSerialNext($type_id, $object_sub_details_id, $object_sub_description_id) {
		...
		if (static::$stmt_object_sub_description_serial === null) {
			
			DB::setConnection(null, DB::MODE_CONNECT_SECONDARY); // Use secondary connection, outside any transaction
			
			if (DB::ENGINE_IS_MYSQL) {
			
				static::$stmt_object_sub_description_serial[0] = DB::prepare("
					UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')." SET
						value_type_serial = (@num_serial := COALESCE(value_type_serial, 0) + 1)
					WHERE id = ".DBStatement::assign('object_sub_description_id', 'i')."
						AND object_sub_details_id = ".DBStatement::assign('object_sub_details_id', 'i')."
				");
				static::$stmt_object_sub_description_serial[1] = DB::prepare("
					SELECT @num_serial
				");
			} else {
				
				static::$stmt_object_sub_description_serial = DB::prepare("
					UPDATE ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')." SET
						value_type_serial = COALESCE(value_type_serial, 0) + 1
					WHERE id = ".DBStatement::assign('object_sub_description_id', 'i')."
						AND object_sub_details_id = ".DBStatement::assign('object_sub_details_id', 'i')."
		...
		if (DB::ENGINE_IS_MYSQL) {
			...
			static::$stmt_object_sub_description_serial[0]->bindParameters(['object_sub_details_id' => (int)$object_sub_details_id, 'object_sub_description_id' => (int)$object_sub_description_id]);
			static::$stmt_object_sub_description_serial[0]->execute();
			
			$res = static::$stmt_object_sub_description_serial[1]->execute();
		} else {
			
			static::$stmt_object_sub_description_serial->bindParameters(['object_sub_details_id' => (int)$object_sub_details_id, 'object_sub_description_id' => (int)$object_sub_description_id]);
			$res = static::$stmt_object_sub_description_serial->execute();
		}
	...
	public static function setTypesObjectPath($arr_type_ids, $path) {
		...
		$stmt_insert_path = DB::prepare("INSERT INTO ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_'.($path == 'name' ? 'NAME' : 'SEARCH').'_PATH')."
			(type_id, ref_type_id, ref_object_description_id, ref_object_sub_details_id, org_object_description_id, org_object_sub_details_id, is_reference, sort)
				VALUES
			(
				".DBStatement::assign('type_id', 'i').",
				".DBStatement::assign('ref_type_id', 'i').",
				".DBStatement::assign('ref_object_description_id', 'i').",
				".DBStatement::assign('ref_object_sub_details_id', 'i').",
				".DBStatement::assign('org_object_description_id', 'i').",
				".DBStatement::assign('org_object_sub_details_id', 'i').",
				".DBStatement::assign('is_reference', 'i').",
				".DBStatement::assign('sort', 'i')."
			)
		");
		
		$stmt_select_use_name = DB::prepare("SELECT nodegoat_t.id
				FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." nodegoat_t
			WHERE id = ".DBStatement::assign('ref_type_id', 'i')."
				AND use_object_name = TRUE
		");
		$stmt_select_object_descriptions = DB::prepare("SELECT nodegoat_to_des.*,
			CASE WHEN nodegoat_to_des.value_type_base = 'reference_mutable' THEN
				(SELECT ".DBFunctions::group2String(DBFunctions::castAs('nodegoat_to_des_ref.ref_type_id', DBFunctions::CAST_TYPE_STRING), ',', 'ORDER BY nodegoat_to_des_ref.sort')."
					FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." nodegoat_to_des_ref
				WHERE nodegoat_to_des_ref.id_id = nodegoat_to_des.id)
			ELSE ".DBFunctions::castAs('nodegoat_to_des.ref_type_id', DBFunctions::CAST_TYPE_STRING)." END AS ref_type_id
				FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." nodegoat_to_des
			WHERE nodegoat_to_des.type_id = ".DBStatement::assign('ref_type_id', 'i')."
				AND nodegoat_to_des.in_".($path == 'name' ? 'name' : 'search')." = TRUE
			ORDER BY nodegoat_to_des.type_id ASC, nodegoat_to_des.sort ASC
		");
		$stmt_select_object_sub_descriptions = DB::prepare("SELECT nodegoat_to_det.type_id, nodegoat_tos_des.*,
			CASE WHEN nodegoat_tos_des.value_type_base = 'reference_mutable' THEN
				(SELECT ".DBFunctions::group2String(DBFunctions::castAs('nodegoat_tos_des_ref.ref_type_id', DBFunctions::CAST_TYPE_STRING), ',', 'ORDER BY nodegoat_tos_des_ref.sort')."
					FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DESCRIPTIONS')." nodegoat_tos_des_ref
				WHERE nodegoat_tos_des_ref.id_id = nodegoat_tos_des.id)
			ELSE ".DBFunctions::castAs('nodegoat_tos_des.ref_type_id', DBFunctions::CAST_TYPE_STRING)." END AS ref_type_id
			...
			WHERE nodegoat_to_det.type_id = ".DBStatement::assign('ref_type_id', 'i')."
				AND nodegoat_tos_des.in_".($path == 'name' ? 'name' : 'search')." = TRUE
			ORDER BY nodegoat_to_det.type_id ASC, nodegoat_to_det.sort ASC, nodegoat_tos_des.sort ASC
		");
		
		$num_limiting_sequence = (Settings::get('data_model_path_limiting_sequence', $path) ?? static::$num_path_limiting_sequence);
		$num_limiting_references = (Settings::get('data_model_path_limiting_references', $path) ?? static::$num_path_limiting_references);
			
		$func_insert_object_description_name_ids = function($ref_type_id, $org_object_description_id, $org_object_sub_details_id, &$arr_path_types, &$num_limit = false) use (&$cur_type_id, &$num_sequence, &$func_insert_object_description_name_ids, $path, $stmt_insert_path, $stmt_select_use_name, $stmt_select_object_descriptions, $stmt_select_object_sub_descriptions, $num_limiting_sequence, $num_limiting_references) {
			
			$str_identifier = (int)$ref_type_id.'_'.(int)$org_object_description_id.'_'.(int)$org_object_sub_details_id;
			
			if (isset($arr_path_types[$str_identifier]) || $num_limit === 0) { // Prevent recursion
				return;
			}
			
			$is_path_origin = ($org_object_description_id == 0);
			$arr_path_types[$str_identifier] = true;
			$is_limit_owner = false;
			
			if ($num_limit === true) {
				
				$is_limit_owner = true;
				$num_limit = $num_limiting_references;
			}
			
			$stmt_select_use_name->bindParameters(['ref_type_id' => $ref_type_id]);
			$res = $stmt_select_use_name->execute();
			
			if ($res->getRowCount()) {
				
				// Store object's own name
				
				if ($num_limit !== false && !$is_limit_owner) {
					$num_limit--;
				}
				
				$stmt_insert_path->bindParameters([
					'type_id' => $cur_type_id, 'ref_type_id' => $ref_type_id,
					'ref_object_description_id' => 0, 'ref_object_sub_details_id' => 0,
					'org_object_description_id' => $org_object_description_id, 'org_object_sub_details_id' => $org_object_sub_details_id,
					'is_reference' => 0, 'sort' => $num_sequence
				]);
				$res_insert = $stmt_insert_path->execute();

				$num_sequence++;
			}
			
			$stmt_select_object_descriptions->bindParameters(['ref_type_id' => $ref_type_id]);
			$res = $stmt_select_object_descriptions->execute();
			
			while ($arr_row = $res->fetchAssoc()) {
				
				$new_ref_type_id = $arr_row['ref_type_id'];
				
				if ($new_ref_type_id) {
					
					$is_reference = 1; // Store description when only link
					if (strpos($new_ref_type_id, ',') !== false) {
						$new_ref_type_id = explode(',', $new_ref_type_id);
					}
				} else {
					$is_reference = 0; // Store description as name reference
				}
				
				if ($num_limit !== false) {
					
					if ($is_limit_owner) { // Make sure to at least include all non-reference fields
				...
				$stmt_insert_path->bindParameters([
					'type_id' => $cur_type_id, 'ref_type_id' => $arr_row['type_id'],
					'ref_object_description_id' => $arr_row['id'], 'ref_object_sub_details_id' => 0,
					'org_object_description_id' => $org_object_description_id, 'org_object_sub_details_id' => $org_object_sub_details_id,
					'is_reference' => $is_reference, 'sort' => $num_sequence
				]);
				$res_insert = $stmt_insert_path->execute();
				...
				if ($is_path_origin) { // Reset the path's trace only at the initial/departing Type.
					$arr_path_types = [];
				}
					
				if (is_array($new_ref_type_id)) {
					
					foreach ($new_ref_type_id as $cur_ref_type_id) {
						
						if ($num_limit === false) {
							$num_use_limit = true; // Initialise limit references
						} else {
							$num_use_limit =& $num_limit;
						}
						
						$func_insert_object_description_name_ids($cur_ref_type_id, $arr_row['id'], 0, $arr_path_types, $num_use_limit);
					}
				} else {
					
					if ($num_limit === false) {
						$num_use_limit = ($num_sequence > $num_limiting_sequence ? true : $num_limit); // Initialise limit references?
					} else {
						$num_use_limit =& $num_limit;
					}
					
					$func_insert_object_description_name_ids($new_ref_type_id, $arr_row['id'], 0, $arr_path_types, $num_use_limit);
				}
			}
			
			$stmt_select_object_sub_descriptions->bindParameters(['ref_type_id' => $ref_type_id]);
			$res = $stmt_select_object_sub_descriptions->execute();
			...
			while ($arr_row = $res->fetchAssoc()) {
				...
				$stmt_insert_path->bindParameters([
					'type_id' => $cur_type_id, 'ref_type_id' => $arr_row['type_id'],
					'ref_object_description_id' => $arr_row['id'], 'ref_object_sub_details_id' => $arr_row['object_sub_details_id'],
					'org_object_description_id' => $org_object_description_id, 'org_object_sub_details_id' => $org_object_sub_details_id,
					'is_reference' => $is_reference, 'sort' => $num_sequence
				]);
				$res_insert = $stmt_insert_path->execute();
				...
				if ($is_path_origin) { // Reset the path trace's only at the initial/departing Type.
					$arr_path_types = [];
				}
				...
				if (is_array($new_ref_type_id)) {
					...
					foreach ($new_ref_type_id as $cur_ref_type_id) {
						...
						$func_insert_object_description_name_ids($cur_ref_type_id, $arr_row['id'], $arr_row['object_sub_details_id'], $arr_path_types, $num_use_limit);
					}
				} else {
					...
					$func_insert_object_description_name_ids($new_ref_type_id, $arr_row['id'], $arr_row['object_sub_details_id'], $arr_path_types, $num_use_limit);
				}
			}
		};
		...
		$stmt_select_types = DB::prepare("SELECT DISTINCT type_id
				FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_'.($path == 'name' ? 'NAME' : 'SEARCH').'_PATH')."
			WHERE ref_type_id = ".DBStatement::assign('type_id', 'i')."
				AND type_id != ".DBStatement::assign('type_id2', 'i')."
		");
		
		foreach ($arr_type_ids as $type_id) {
			
			$stmt_select_types->bindParameters(['type_id' => $type_id, 'type_id2' => $type_id]);
			$res = $stmt_select_types->execute();
		...
		$stmt_delete_types = DB::prepare("DELETE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_'.($path == 'name' ? 'NAME' : 'SEARCH').'_PATH')."
			WHERE type_id = ".DBStatement::assign('type_id', 'i')."
		");
		
		foreach ($arr_collect_types_ids as $cur_type_id) {
			
			$stmt_delete_types->bindParameters(['type_id' => $cur_type_id]);
			$res_delete = $stmt_delete_types->execute();
	...
	public function getTypeDefinitionID($type_id, $value) {
		...
		if ($is_name) {
			...
			if ($arr_type_set) {
				...
				foreach ($arr_type_set['definitions'] as $arr_definition) {
					...
					if ($arr_definition['definition_name'] != $value) {
						continue;
					}
					
					$definition_id = $arr_definition['definition_id'];
					break;
				}
			}		
			
			if (!$definition_id) {
				// Nothing to be resolved, Type Definitions are not referenced
			}
		} else {
			
			$definition_id = ($arr_type_set['definitions'][$value]['definition_id'] ?? null);
		}
	...
	public function getTypeObjectDescriptionID($type_id, $value) {
		...
		if ($is_name) {
			...
			$object_description_id = ($arr_type_set['object_descriptions'][$value]['object_description_id'] ?? null);
		}
	...
	public function getTypeObjectSubDetailsID($type_id, $value) {
		...
		if ($is_name) {
			...
			$object_sub_details_id = ($arr_type_set['object_sub_details'][$value]['object_sub_details']['object_sub_details_id'] ?? null);
		}
	...
	public function getTypeObjectSubDescriptionID($type_id, $object_sub_details_id, $value) {
		...
		if ($is_name) {
			...
			$object_sub_description_id = ($arr_type_set['object_sub_details'][$object_sub_details_id]['object_sub_descriptions'][$value]['object_sub_description_id'] ?? null);
		}
	...
	public function getReferencingTypesMode($arr_type_ids) {
	...
	public static function getTypes($type_id = false, $class = false) {
		...
		if ($type_id) {
			...
			if (is_array($type_id)) {
				$arr_type_ids = arrParseRecursive($type_id, TYPE_INTEGER);
	...
	public static function getSystemTypes($type_id = false) {
		...
		if ($type_id && $type_id == -4) { // Do not list, only return when requested
			...
			$arr[-4] = [ // Reversal prototype
				'id' => -4,
				'name' => getLabel('lbl_nodegoat_reversal'),
				'class' => static::TYPE_CLASS_SYSTEM,
				'mode' => static::TYPE_MODE_DEFAULT,
				'use_object_name' => true,
				'object_name_in_overview' => true
	...
	public static function getTypeSet($type_id) {
		...
		$system_reversal_type_id = static::getSystemTypeID('reversal');
		$system_reversal_module_object_description_id = static::getSystemTypeObjectDescriptionID($system_reversal_type_id, 'module');
	
		$arr = ['type' => null, 'definitions' => [], 'object_descriptions' => [], 'object_sub_details' => []];
		...
		$arr_res = DB::queryMulti("
			SELECT nodegoat_t.*,
				...
				ORDER BY nodegoat_t_des.sort ASC;
			...
			SELECT nodegoat_t.id,
					nodegoat_to_des.id AS object_description_id, nodegoat_to_des.id_id AS object_description_id_id, nodegoat_to_des.name AS object_description_name,
						...
						CASE
							WHEN nodegoat_to_des.value_type_base = 'reversal' THEN
								CASE
									WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN CASE
										WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." WHERE type_id = nodegoat_to_des.ref_type_id AND id_id = ".$system_reversal_module_object_description_id." AND value_type_settings LIKE '%\"resource_path\":true%') THEN 'reversed_collection_resource_path'
										ELSE 'reversed_collection'
									END
						...
						CASE 
							...
							WHEN nodegoat_to_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN TRUE
							ELSE FALSE
				...
				FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." nodegoat_t
				JOIN ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." nodegoat_to_des ON (nodegoat_to_des.type_id = nodegoat_t.id)
				WHERE nodegoat_t.id = ".(int)$type_id."
				ORDER BY nodegoat_to_des.id_id IS NULL DESC, nodegoat_to_des.sort ASC;
			...
			SELECT nodegoat_t.id,
					...
					nodegoat_tos_des.id AS object_sub_description_id, nodegoat_tos_des.id_id AS object_sub_description_id_id, nodegoat_tos_des.name AS object_sub_description_name,
					...
					CASE
						WHEN nodegoat_tos_des.value_type_base = 'reversal' THEN
							CASE
								WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN CASE
									WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." WHERE type_id = nodegoat_tos_des.ref_type_id AND id_id = ".$system_reversal_module_object_description_id." AND value_type_settings LIKE '%\"resource_path\":true%') THEN 'reversed_collection_resource_path'
									ELSE 'reversed_collection'
								END
								ELSE 'reversed_classification'
							END
					...
					CASE 
						...
						WHEN nodegoat_tos_des.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN TRUE
						ELSE FALSE
				...
				WHERE nodegoat_t.id = ".(int)$type_id."
				ORDER BY nodegoat_tos_det.sort ASC, nodegoat_tos_des.id_id IS NULL DESC, nodegoat_tos_des.sort ASC;
		");
		...
		while ($arr_row = $arr_res[1]->fetchAssoc()) {
			...
			if ($arr['type']['class'] == static::TYPE_CLASS_REVERSAL) {
				
				$arr_system_type_set = static::getSystemTypeSet($system_reversal_type_id);

				if ($arr['type']['mode'] & static::TYPE_MODE_REVERSAL_CLASSIFICATION && $arr_row['object_description_id_id'] == static::getSystemTypeObjectDescriptionID($system_reversal_type_id, 'reference') && $arr_row['object_description_ref_type_id']) {
				
					$arr_system_object_description = $arr_system_type_set['object_descriptions'][static::getSystemTypeObjectDescriptionID($system_reversal_type_id, 'reference')];
				...
				} else if ($arr['type']['mode'] & static::TYPE_MODE_REVERSAL_COLLECTION && $arr_row['object_description_id_id'] == $system_reversal_module_object_description_id) {
				
					$arr_system_object_description = $arr_system_type_set['object_descriptions'][$system_reversal_module_object_description_id];

					$arr_system_object_description['object_description_value_type_settings'] = ($arr_row['object_description_value_type_settings'] ? json_decode($arr_row['object_description_value_type_settings'], true) : []);
					
					$arr_row = $arr_system_object_description;
				} else {
					continue;
				}
			} else if ($arr_row['object_description_id_id']) {
				
				if ($arr_row['object_description_ref_type_id']) {
					
					$s_arr = &$arr['object_descriptions'][$arr_row['object_description_id_id']];
					
					if (!is_array($s_arr['object_description_ref_type_id'])) {
						$s_arr['object_description_ref_type_id'] = [];
					}						
					$s_arr['object_description_ref_type_id'][$arr_row['object_description_id']] = $arr_row['object_description_ref_type_id'];
		...
		if ($arr['type']['class'] == StoreType::TYPE_CLASS_REVERSAL) {
			...
			if (!isset($arr['object_descriptions'][$system_reversal_module_object_description_id])) {
			
				$arr_system_object_description = static::getSystemTypeSet($system_reversal_type_id);
				$arr_system_object_description = $arr_system_object_description['object_descriptions'][$system_reversal_module_object_description_id];
				
				$arr['object_descriptions'][$system_reversal_module_object_description_id] = $arr_system_object_description;
			}
		}
		...
		while ($arr_row = $arr_res[2]->fetchAssoc()) {
			...
			if ($arr_row['object_sub_description_id_id']) {
				
				if ($arr_row['object_sub_description_ref_type_id']) {
					
					$s_arr = &$arr['object_sub_details'][$arr_row['object_sub_details_id']]['object_sub_descriptions'][$arr_row['object_sub_description_id_id']];
					
					if (!is_array($s_arr['object_sub_description_ref_type_id'])) {
						$s_arr['object_sub_description_ref_type_id'] = [];
					}						
					$s_arr['object_sub_description_ref_type_id'][$arr_row['object_sub_description_id']] = $arr_row['object_sub_description_ref_type_id'];
					
					unset($s_arr);
				}
				continue;
			} else if ($arr_row['object_sub_description_id']) {
			...
			case -2: // Ingest
				...
				$arr['object_descriptions'] = [
					-3 => [
						...
						'object_description_value_type_base' => 'module',
					...
					-5 => [ // Store processing state
						...
						'object_description_value_type_base' => 'integer',
						'object_description_value_type' => 'integer',
						'object_description_has_multi' => false,
						'object_description_in_name' => false,
						'object_description_is_required' => false,
			...
			case -4: // Reversal prototype
				...
				$arr['object_descriptions'] = [
					-9 => [
						...
						'object_description_value_type_base' => false,
					...
					-10 => [
						...
						'object_description_name' => getLabel('lbl_nodegoat_reversal'),
						'object_description_value_type_base' => 'module',
	...
	public static function getSystemTypeID($name, bool $flip = false) {
		...
		if (isset(static::$arr_system_type_ids[$flip])) {
	...
	public static function getSystemTypeObjectDescriptionID($type_id, $name, bool $flip = false) {
		...
		if (isset(static::$arr_system_type_object_description_ids[$flip])) {
	...
	public static function getTypeSetFlatMap($type_id, $arr_options = ['object_id' => false, 'object_name' => false, 'references' => true, 'reversals' => false]) {
		...
		if ($arr_options['object_id']) {
			$arr['object-id'] = ['id' => 'object-id', 'name' => getLabel('lbl_nodegoat_id')]; 
		}
				
		if ($arr_options['object_name']) {
			$arr['object-name'] = ['id' => 'object-name', 'name' => 'Object Name (Full)']; 
		}
		if ($arr_type_set['type']['use_object_name']) {
			$arr['object-name_plain'] = ['id' => 'object-name_plain', 'name' => 'Object Name']; 
		}
		...
		foreach ($arr_type_set['object_descriptions'] as $object_description_id => $arr_object_description) {
			...
			if ($arr_object_description['object_description_value_type_base'] == 'reversal') {
				...
				if (!$arr_options['reversals']) {
					continue;
				}

				if ($arr_object_description['object_description_value_type'] == 'reversed_classification' && !keyIsUncontested('references', $arr_options)) {
					continue;
				}
			} else if ($arr_object_description['object_description_ref_type_id'] && !keyIsUncontested('references', $arr_options)) {
			...
			$arr['object_description-'.$object_description_id] = ['id' => 'object_description-'.$object_description_id, 'name' => $arr_object_description['object_description_name'], 'ref_type_id' => $arr_object_description['object_description_ref_type_id'], 'is_appendable' => $is_appendable, 'has_multi' => $arr_object_description['object_description_has_multi'], 'is_mutable_ref_type_id' => $is_mutable]; 
		}
					...
					if ($arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_object_sub_details_id_locked']) {
						...
						if (keyIsUncontested('references', $arr_options)) {
							...
							$arr[$str_identifier.'-location_ref_type_id-type_lock-'.$arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_type_id']] = [
								...
								'ref_type_id' => $arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_type_id'],
								'object_sub_details_id' => $arr_object_sub_details['object_sub_details']['object_sub_details_location_ref_object_sub_details_id'],
								'is_mutable_object_sub_details_id' => true,
								'is_location_reference' => true,
								'is_appendable' => $is_appendable
							]; 
						}
				...
				if ($arr_object_sub_details['object_sub_descriptions']) {
					...
					foreach ($arr_object_sub_details['object_sub_descriptions'] as $object_sub_description_id => $arr_object_sub_description) {
						...
						if ($arr_object_sub_description['object_sub_description_value_type_base'] == 'reversal') {
							...
							if ($arr_object_sub_description['object_sub_description_value_type'] == 'reversed_classification' && !keyIsUncontested('references', $arr_options)) {
								continue;
							}
						} else if ($arr_object_sub_description['object_sub_description_ref_type_id'] && !keyIsUncontested('references', $arr_options)) {
		...
		foreach ($arr_map as $element_identifier => $arr_select) {
			...
			if ($arr_element[0] == 'object') {
				...
				if ($has_select) {
					...
					if (isset($arr_select['use_value'])) {
						$arr_collect['object_description_value'] = $arr_select['use_value'];
					}
					if (isset($arr_select['use_reference'])) {
						$arr_collect['object_description_reference'] = $arr_select['use_reference'];
					}
					if (isset($arr_select['use_reference_value'])) {
						$arr_collect['object_description_reference_value'] = $arr_select['use_reference_value'];
					}
				...
				switch ($str_element) {
					...
					case 'object_sub_description':
						...
						if ($has_select) {
							...
							if (isset($arr_select['use_value'])) {
								$arr_collect['object_sub_description_value'] = $arr_select['use_value'];
							}
							if (isset($arr_select['use_reference'])) {
								$arr_collect['object_sub_description_reference'] = $arr_select['use_reference'];
							}
							if (isset($arr_select['use_reference_value'])) {
								$arr_collect['object_sub_description_reference_value'] = $arr_select['use_reference_value'];
							}
	...
	public static function getTypeObjectDescriptionValueType($type_id, $object_description_id) {
		...
		if (!$type_id) {
			...
			if (!isset($arr_type_ids[$object_description_id])) {
				...
				$arr_type_ids[$object_description_id] = (int)$arr_row[0];
			}
			
			$type_id = $arr_type_ids[$object_description_id];
		}
		
		$arr_type_set = self::getTypeSet($type_id);
		$arr = $arr_type_set['object_descriptions'][$object_description_id];
		
		$str_value_type = $arr['object_description_value_type'];

		return $str_value_type;
	}
	
	public static function getTypeObjectSubDescriptionValueType($type_id, $object_sub_details_id, $object_sub_description_id) {
		...
		if (!$type_id) {
			...
			if (!isset($arr_type_ids[$object_sub_details_id])) {
				
				$res = DB::query("				
					SELECT type_id
						FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DETAILS')."
					WHERE id = ".(int)$object_sub_details_id."
				");
				
				$arr_row = $res->fetchRow();
				$arr_type_ids[$object_sub_details_id] = (int)$arr_row[0];
			}
			
			$type_id = $arr_type_ids[$object_sub_details_id];
		}

		$arr_type_set = self::getTypeSet($type_id);
		$arr = $arr_type_set['object_sub_details'][$object_sub_details_id]['object_sub_descriptions'][$object_sub_description_id];

		$str_value_type = $arr['object_sub_description_value_type'];
	...
	public static function isAppendableTypeObjectDescription($type_id, $object_description_id) {
		...
		$str_value_type_base = $arr['object_description_value_type_base'];
		...
		if (!$str_value_type_base || $str_value_type_base == 'integer' || strpos($str_value_type_base, 'text') !== false) {
			return true;
		}
	...
	public static function isAppendableTypeObjectSubDescription($type_id, $object_sub_details_id, $object_sub_description_id) {
		...
		$str_value_type_base = $arr['object_sub_description_value_type_base'];
		$has_multi = false;
	...
	public static function getTypeVersionableIDs($type_id) {

		$arr_type_set = self::getTypeSet($type_id);
		
		$arr_type_versionable = [];
		
		foreach ($arr_type_set['object_descriptions'] as $object_description_id => $arr_object_description) {
		
			if ($arr_object_description['object_description_value_type_base'] == 'reversal' || isset($arr_type_set['type']['include_referenced']['object_descriptions'][$object_description_id])) {
				continue;
			}
			
			$arr_type_versionable['object_descriptions'][$object_description_id] = $object_description_id;
		}
		
		foreach ($arr_type_set['object_sub_details'] as $object_sub_details_id => $arr_object_sub_details) {
			
			if (isset($arr_type_set['type']['include_referenced']['object_sub_details'][$object_sub_details_id])) {
				continue;
			}
			
			$arr_type_versionable['object_sub_details'][$object_sub_details_id] = [];
			...
			foreach ($arr_object_sub_details['object_sub_descriptions'] as $object_sub_description_id => $arr_object_sub_description) {
				...
				$arr_type_versionable['object_sub_details'][$object_sub_details_id][$object_sub_description_id] = $object_sub_description_id;
				$arr_type_versionable['object_sub_descriptions'][$object_sub_description_id] = $object_sub_description_id;
			}
		}
		
		return $arr_type_versionable;
	}
	
	public static function hasReferenceTypeID($reference_type_id, $check_type_id) {
		...
		if (!is_array($reference_type_id)) {
			return ($reference_type_id == $check_type_id);
		}
		
		return in_array($check_type_id, $reference_type_id);
	}
	
	public static function getTypeObjectPath($path, $type_id) {
		
		$is_mutable = is_array($type_id);
		
		if ($is_mutable) {
			$type_id =  arrParseRecursive($type_id, TYPE_INTEGER);
			$str_identifier = implode('_', $type_id);
		} else {
			$type_id = (int)$type_id;
			$str_identifier = $type_id;
		}

		if (isset(self::$arr_types_object_path_storage[$path][$str_identifier])) {
			return self::$arr_types_object_path_storage[$path][$str_identifier];
		}
		
		$system_reversal_module_object_description_id = static::getSystemTypeObjectDescriptionID(static::getSystemTypeID('reversal'), 'module');
		$num_path_limiting_references = (Settings::get('data_model_path_limiting_references', $path) ?? static::$num_path_limiting_references);
		
		if ($is_mutable) {
			
			$sql_type_id = 'IN ('.implode(',', $type_id).') AND nodegoat_to_path.sort < '.$num_path_limiting_references; // This should get a valid sort on a first field
			$sql_sort = DBFunctions::fieldToPosition('nodegoat_to_path.type_id', $type_id).', ';
		} else {
		...
		$res = DB::query("SELECT
				...
				WHEN nodegoat_tos_des_ref.id != 0 THEN
					CASE
						WHEN nodegoat_tos_des_ref.value_type_base = 'reversal' THEN
							CASE
								WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des_ref.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN CASE
									WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." WHERE type_id = nodegoat_tos_des_ref.ref_type_id AND id_id = ".$system_reversal_module_object_description_id." AND value_type_settings LIKE '%\"resource_path\":true%') THEN 'reversed_collection_resource_path'
									ELSE 'reversed_collection'
								END
								ELSE 'reversed_classification'
							END
				...
				ELSE
					CASE
						WHEN nodegoat_to_des_ref.value_type_base = 'reversal' THEN
							CASE
								WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des_ref.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN CASE
									WHEN EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_DESCRIPTIONS')." WHERE type_id = nodegoat_to_des_ref.ref_type_id AND id_id = ".$system_reversal_module_object_description_id." AND value_type_settings LIKE '%\"resource_path\":true%') THEN 'reversed_collection_resource_path'
									ELSE 'reversed_collection'
								END
								ELSE 'reversed_classification'
							END
			...
			CASE
				WHEN nodegoat_tos_des_ref.id != 0 THEN
					CASE 
						...
						WHEN nodegoat_tos_des_ref.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_tos_des_ref.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN TRUE
						ELSE FALSE
					END
				ELSE
					CASE 
						WHEN nodegoat_to_des_ref.value_type_base = 'text_tags' THEN TRUE
						WHEN nodegoat_to_des_ref.value_type_base = 'reversal' AND EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPES')." WHERE id = nodegoat_to_des_ref.ref_type_id AND mode & ".static::TYPE_MODE_REVERSAL_COLLECTION." != 0) THEN TRUE
						ELSE FALSE
					END
			END AS is_dynamic,
			CASE
				WHEN nodegoat_tos_des_ref.id != 0 THEN
					EXISTS (SELECT TRUE FROM ".DB::getTable('DEF_NODEGOAT_TYPE_OBJECT_SUB_DETAILS')." WHERE id = nodegoat_tos_des_ref.object_sub_details_id AND is_single = FALSE)
				ELSE
					nodegoat_to_des_ref.has_multi
			END AS has_multi
			...
			WHERE nodegoat_to_path.type_id ".$sql_type_id."
			ORDER BY ".$sql_sort."sort ASC, is_reference DESC, org_object_description_id, org_object_sub_details_id
		");
		...
		while ($arr_row = $res->fetchAssoc()) {
			...
			$arr_row['has_multi'] = DBFunctions::unescapeAs($arr_row['has_multi'], DBFunctions::TYPE_BOOLEAN);
			
			$arr[] = $arr_row;
			
			if ($is_mutable && $arr_row['sort'] == $num_path_limiting_references-1 && $arr_row['is_reference']) { // Make sure the referencing type's last section is not a reference
				...
				while ($num_key) {
					...
					$arr_check = $arr[$num_key];
					
					if ($arr_check['type_id'] != $arr_row['type_id'] || !$arr_check['is_reference']) {
						$num_key = false;
					}						
				}
			}
		...
		self::$arr_types_object_path_storage[$path][$str_identifier] = $arr;	
	...
	public static function getTypeClassName($num_class, $do_plural = false) {
		
		$str_class = '';
		
		if ($num_class == static::TYPE_CLASS_CLASSIFICATION) {
			$str_class = getLabel($do_plural ? 'lbl_nodegoat_classifications' : 'lbl_nodegoat_classification');
		} else if ($num_class == static::TYPE_CLASS_REVERSAL) {
			$str_class = getLabel($do_plural ? 'lbl_nodegoat_reversals' : 'lbl_nodegoat_reversal');
		} else {
			$str_class = getLabel($do_plural ? 'lbl_nodegoat_types' : 'lbl_nodegoat_type');
		}

		return $str_class;
	}
	
	public static function getTypeClassObjectName($num_class, $do_plural = false) {
		
		$str_name = '';
		
		if ($num_class == static::TYPE_CLASS_CLASSIFICATION) {
			$str_name = getLabel($do_plural ? 'lbl_categories' : 'lbl_category');
		} else if ($num_class == static::TYPE_CLASS_REVERSAL) {
			$str_name = getLabel($do_plural ? 'lbl_processes' : 'lbl_process');
		} else {
			$str_name = getLabel($do_plural ? 'lbl_objects' : 'lbl_object');
		}
	...
	public static function getValueTypesBase() {
		...
		$arr_value_types_base = [
			'type' => ['id' => 'type', 'name' => getLabel('lbl_reference').': '.getLabel('lbl_nodegoat_type'), 'name_base' => getLabel('lbl_nodegoat_type'), 'has_support_multi' => true],
			'classification' => ['id' => 'classification', 'name' => getLabel('lbl_reference').': '.getLabel('lbl_nodegoat_classification'), 'name_base' => getLabel('lbl_nodegoat_classification'), 'has_support_multi' => true],
			'reference_mutable' => ['id' => 'reference_mutable', 'name' => getLabel('lbl_reference').': '.getLabel('lbl_reference_mutable'), 'name_base' => getLabel('lbl_reference_mutable'), 'has_support_multi' => true],
			'reversal' => ['id' => 'reversal', 'name' => getLabel('lbl_reference').': '.getLabel('lbl_nodegoat_reversal'), 'name_base' => getLabel('lbl_nodegoat_reversal'), 'has_support_multi' => true],
			'' => ['id' => '', 'name' => getLabel('unit_data_string'), 'name_base' => getLabel('unit_data_string'), 'has_support_multi' => true, 'has_support_identifier' => true],
			'integer' => ['id' => 'integer', 'name' => getLabel('unit_data_number'), 'name_base' => getLabel('unit_data_number'), 'has_support_multi' => true],
			...
			'serial_string' => ['id' => 'serial_string', 'name' => getLabel('unit_data_serial_string'), 'name_base' => getLabel('unit_data_serial'), 'has_support_multi' => true, 'has_support_identifier' => true],
			...
			'vector' => ['id' => 'vector', 'name' => getLabel('unit_data_vector'), 'name_base' => getLabel('unit_data_vector'), 'support' => (Settings::get('value_type_vector', 'support') ?? false), 'has_support_multi' => true, 'has_support_text' => false],
	...
	public static function getValueTypes() {
		...
		self::$arr_value_types = [
			'' => ['id' => '', 'name' => getLabel('unit_data_string'), 'base' => '', 'table' => '', 'value' => 'value'],
			'reference' => ['id' => 'reference', 'name' => getLabel('lbl_reference'), 'base' => 'type', 'table' => '_references', 'value' => 'ref_object_id'],
			'type' => ['id' => 'type', 'name' => getLabel('lbl_nodegoat_type'), 'base' => 'type', 'table' => '_references', 'value' => 'ref_object_id'],
			'classification' => ['id' => 'classification', 'name' => getLabel('lbl_nodegoat_classification'), 'base' => 'classification', 'table' => '_references', 'value' => 'ref_object_id'],
			'reference_mutable' => ['id' => 'reference_mutable', 'name' => getLabel('lbl_reference_mutable'), 'base' => 'reference_mutable', 'table' => '_references', 'value' => 'ref_object_id'],
			'reversed_classification' => ['id' => 'reversed_classification', 'name' => getLabel('lbl_reversed_classification'), 'base' => 'reversal', 'table' => '_references', 'value' => 'ref_object_id'],
			'reversed_collection' => ['id' => 'reversed_collection', 'name' => getLabel('lbl_reversed_collection'), 'base' => 'reversal', 'table' => '_references', 'value' => 'ref_object_id', 'purpose' => ['name' => ['table' => '', 'value' => 'value_text'], 'search' => ['table' => '', 'value' => 'value_text'], 'view' => null]],
			'reversed_collection_resource_path' => ['id' => 'reversed_collection_resource_path', 'name' => getLabel('lbl_reversed_collection_resource_path'), 'base' => 'reversal', 'table' => '_references', 'value' => 'ref_object_id', 'purpose' => ['name' => ['table' => '', 'value' => 'value_text'], 'search' => ['table' => '', 'value' => 'value_text'], 'view' => ['table' => '', 'value' => 'value_text']]],
			'integer' => ['id' => 'integer', 'name' => getLabel('unit_data_number'), 'base' => 'integer', 'table' => '', 'value' => 'value_int'],
			'numeric' => ['id' => 'numeric', 'name' => getLabel('unit_data_numeric_decimal'), 'base' => 'numeric', 'table' => '', 'value' => 'value_int'],
			'text' => ['id' => 'text', 'name' => getLabel('unit_data_text'), 'base' => 'text', 'table' => '', 'value' => 'value_text'],
			'text_layout' => ['id' => 'text_layout', 'name' => getLabel('unit_data_text').' ('.getLabel('lbl_layout').')', 'base' => 'text_layout', 'table' => '', 'value' => 'value_text'],
			'text_tags' => ['id' => 'text_tags', 'name' => getLabel('unit_data_text').' ('.getLabel('lbl_tags').' & '.getLabel('lbl_layout').')', 'base' => 'text_tags', 'table' => '', 'value' => 'value_text'],
			'boolean' => ['id' => 'boolean', 'name' => getLabel('unit_data_boolean_true_false'), 'base' => 'boolean', 'table' => '', 'value' => 'value_int'],
			'date' => ['id' => 'date', 'name' => getLabel('unit_data_date'), 'base' => 'date', 'table' => '', 'value' => 'value_int'],
			'date_cycle' => ['id' => 'date_cycle', 'name' => getLabel('unit_data_date').' '.getLabel('lbl_date_cycle'), 'base' => 'date', 'table' => '', 'value' => 'value_int'],
			'date_compute' => ['id' => 'date_compute', 'name' => getLabel('unit_data_date').' '.getLabel('lbl_date_compute'), 'base' => 'date', 'table' => '', 'value' => 'value_int'],
			'serial_string' => ['id' => 'serial_string', 'name' => getLabel('unit_data_serial_string'), 'base' => 'serial_string', 'table' => '', 'value' => 'value'],
			'media' => ['id' => 'media', 'name' => getLabel('lbl_media'), 'base' => 'media', 'table' => '', 'value' => 'value'],
			'media_external' => ['id' => 'media_external', 'name' => getLabel('lbl_media').' ('.getLabel('lbl_external').')', 'base' => 'media_external', 'table' => '', 'value' => 'value'],
			'module' => ['id' => 'module', 'name' => '', 'base' => 'module', 'table' => '_modules', 'value' => 'object', 'is_module' => true],
			'external' => ['id' => 'external', 'name' => getLabel('lbl_external'), 'base' => 'external', 'table' => '', 'value' => 'value'],
			'vector' => ['id' => 'vector', 'name' => getLabel('unit_data_vector'), 'base' => 'vector', 'table' => '_vectors', 'value' => 'embed', 'value_secondary' => 'embed_normalise', 'do_versioning' => (Settings::get('value_type_vector', 'versioning') ?? false), 'purpose' => ['name' => null, 'search' => ['table' => '_vectors', 'value' => 'embed_normalise'], 'view' => null]],
			'external_module' => ['id' => 'external_module', 'name' => '', 'base' => 'module', 'table' => '_modules', 'value' => 'object', 'is_module' => true, 'module_class' => 'data_ingest'],
			'object_description' => ['id' => 'object_description', 'name' => getLabel('lbl_object_description'), 'base' => 'object_description', 'table' => '', 'value' => 'value'],
			'reconcile_module' => ['id' => 'reconcile_module', 'name' => '', 'base' => 'module', 'table' => '_modules', 'value' => 'object', 'is_module' => true, 'module_class' => 'data_reconcile'],
			'reversal_module' => ['id' => 'reversal_module', 'name' => '', 'base' => 'module', 'table' => '_modules', 'value' => 'object', 'is_module' => true]
		];
	...
	public static function getValueType($str_type, $str_property = false, $str_purpose = false) {
		...
		return ($str_property ? ($arr_value_type[$str_property] ?? null) : $arr_value_type);
	}
	
	public static function isValueTypeValueTextual($str_type, $str_purpose = false) {
		
		$str_value_type_value = static::getValueType($str_type, 'value', $str_purpose);
		
		return ($str_value_type_value == 'value_text' || $str_value_type_value == 'value');
	}
	...
	public static function getValueTypeTables() {
		...
		foreach (self::$arr_value_types as $value_type => $arr_value_type) {
			...
			$arr_tables[$arr_value_type['table']]['value_types_base'][$arr_value_type['base']] = $arr_value_type['base'];
		}	
		
		return $arr_tables;
	}
	
	public static function getValueTypeTablesReferences() {
		...
		foreach (self::$arr_value_types as $value_type => $arr_value_type) {
			...
			if ($arr_value_type['value'] != 'ref_object_id') {
				continue;
			}
				...
				if ($s_arr['selection']) {
					...
					foreach ($s_arr['selection'] as $id => $arr_selected) {
						...
						if (isset($arr_selected['id'])) { // Form
							...
							$pos_dynamic_mutable = strpos($str_id, '_mutable_');
						
							if ($pos_dynamic_mutable !== false) {
								
								$ref_type_id = (int)substr($str_id, $pos_dynamic_mutable + 9);
								$str_id = substr($str_id, 0, $pos_dynamic_mutable);
					...
					foreach ($arr_type_set['object_descriptions'] as $object_description_id => $arr_object_description) {
						...
						$is_mutable = is_array($arr_object_description['object_description_ref_type_id']);
						$in_selection = (isset($arr_selection[$str_id]) || ($is_dynamic && (isset($arr_selection[$str_id.'_reference']) || isset($arr_selection[$str_id.'_value']))));
						...
						if ($in_selection && !static::checkTypeConfigurationUserClearance($arr_type_set, $num_user_clearance, $object_description_id, false, false, self::CLEARANCE_PURPOSE_VIEW)) {
							...
							if ($is_dynamic) {
								...
								if (isset($arr_selection[$str_id.'_reference'])) {
									
									$arr_selection[$str_id.'_reference']['object_description_id'] = $object_description_id;
									$arr_selection[$str_id.'_reference']['use_reference'] = ($arr_selection[$str_id.'_reference']['ref_type_ids'] ?: true);
									$arr_selection[$str_id.'_reference']['use_reference_value'] = true;
									unset($arr_selection[$str_id.'_reference']['ref_type_ids']);
								}
								if (isset($arr_selection[$str_id.'_value'])) {
									
									$arr_selection[$str_id.'_value']['object_description_id'] = $object_description_id;
									$arr_selection[$str_id.'_value']['use_value'] = true;
								}
							} else if ($is_mutable) {
								
								if (isset($arr_selection[$str_id]['ref_type_ids'])) {
										
									$arr_selection[$str_id]['use_reference'] = $arr_selection[$str_id]['ref_type_ids'];
									$arr_selection[$str_id]['use_value'] = true;
									unset($arr_selection[$str_id]['ref_type_ids']);
								}
							}
						}
					}
					...
					foreach ($arr_type_set['object_sub_details'] as $object_sub_details_id => $arr_object_sub_details) {
						...
						if (isset($arr_selection[$str_id])) {
							...
							$arr_selection[$str_id]['attribute'] = 'id';
						}
						...
						if (isset($arr_selection[$str_id])) {
							$arr_selection[$str_id]['object_sub_details_id'] = $object_sub_details_id;
							$arr_selection[$str_id]['attribute'] = 'date_start';
						}
						$str_id = 'object_sub_details_'.$object_sub_details_id.'_date_end';
						if (isset($arr_selection[$str_id])) {
							$arr_selection[$str_id]['object_sub_details_id'] = $object_sub_details_id;
							$arr_selection[$str_id]['attribute'] = 'date_end';
						}
			...
			foreach ($arr_source_type as $type_id => $arr_type) {
				...
				if ($s_arr['selection']) {
					...
					foreach ($arr_type_set['object_descriptions'] as $object_description_id => $arr_object_description) {
						...
						if (isset($arr_selection[$str_id.'_id'])) {
					...
					foreach ($arr_type_set['object_sub_details'] as $object_sub_details_id => $arr_object_sub_details) {
						...
						if (!static::checkTypeConfigurationUserClearance($arr_type_set, $num_user_clearance, false, $object_sub_details_id, false, self::CLEARANCE_PURPOSE_VIEW)) {
						...
						if (isset($arr_selection[$str_id])) {
							$arr_selection[$str_id]['object_sub_details_id'] = $object_sub_details_id;
							$arr_selection[$str_id]['attribute'] = 'date';
						}
						$str_id = 'object_sub_details_'.$object_sub_details_id.'_date_start';
						if (isset($arr_selection[$str_id])) {
							$arr_selection[$str_id]['object_sub_details_id'] = $object_sub_details_id;
				...
				if (!$s_arr['nodegoat_id'] && !$s_arr['filter'] && !$s_arr['id'] && !$s_arr['name'] && !$s_arr['sources'] && !$s_arr['analysis'] && !$s_arr['selection']) {
					unset($arr['types'][$source_path][$type_id]);
				}
			}
			...
			foreach ($arr_paths as $source_path => &$arr_value) {
				...
				foreach ($arr_value as $str_key => $arr_settings) {
					...
					$func_parse_chronology_statement = function($arr_date_settings) {
						...
						FormatTypeObjects::parseChronologyStatementJSON($arr_chronology_statement, $arr_date_settings);
